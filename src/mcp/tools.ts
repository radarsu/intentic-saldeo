...
// The agent's SaldeoSMART tools for ONE connected card. The backend serves them (server.ts, `mcp/<card>`) and the
// daemon mounts that endpoint into every turn granted the card, as a server named by the card's id: one handler in the
// shared backend host rather than a stdio process per session. Which tools exist follows the card's switches, so a card
// that says "no proposing" yields an agent that has no way to propose.
...
export const saldeoMcpServer = (api: SaldeoService, connection: AccountConnection): McpServer => {
    ...
    if (scopes.documents) {
        server.registerTool(
            "saldeo_documents",
            {
                ...
                inputSchema: { ...companyArg, number: z.string().optional().describe("The document number as printed."), nip: z.string().optional().describe("The contractor's NIP, digits only.") },
            },
            async ({ company, number, nip }) => {
                try {
    ...
    if (scopes.propose) {
        ...
        server.registerTool(
            "saldeo_session",
            {
                ...
                inputSchema: { ...sessionArg, all: z.boolean().optional().describe("Every item, decided ones included.") },
            },
            async ({ session, all }) => {
                try {
        ...
        server.registerTool(
            "saldeo_skip",
            {
                ...
                inputSchema: { ...sessionArg, transactionId: z.string(), reason: z.string().min(1) },
            },
